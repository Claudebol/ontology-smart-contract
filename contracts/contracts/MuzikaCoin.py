...
def Deploy():
    ...
    Put(ctx, CBK_SUPPLY_KEY, INIT_SUPPLY * FACTOR)
...
def TotalSupply():
    """
    Gets the total supply for CBK token. The total supply can be changed by
...
def BalanceOf(account):
    """
    Gets the CBK token balance of an account.
...
def Approve(_from, _to, _amount):
    """
    Approves `to` address to withdraw CBK token from the invoker's address. It
    overwrites the previous approval value.

    :param _from: invoker address.
    :param _to: address to approve.
    :param _amount: CBK amount to approve.
    """
...
def Burn(_amount):
    """
    Burns the amount of CBK token from the owner's address.
    :param _amount: CBK amount to burn.
    """
    ctx = GetContext()
...
def _burn(_context, _account, _amount):
    ...
    SafePut(_context, CBK_SUPPLY_KEY, total_supply)
    return True
...
def _totalSupply(_context):
    return Get(_context, CBK_SUPPLY_KEY)
