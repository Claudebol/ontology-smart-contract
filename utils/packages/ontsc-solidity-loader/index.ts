...
const ontscMigrator = require('ontsc-core/lib/commands/migrate')
...
export = function (source, map, meta) {
  let WebpackOptions = getOptions(this) || {}
  validateOptions(schema, WebpackOptions, 'ontsc-solidity-loader')
  ...
  ontscMigrator.run(buildOpts, function (err) {
    if (err) {
      return callback(err)
    } else {
