  ...
  return Promise.resolve().then(function() {
    ...
    if (contents.length) {
      var err = "Something already exists at the destination. " +
                "`ontsc init` and `ontsc unbox` must be executed in an empty folder. " +
  ...
  return new Promise(function(accept, reject) {
    ...
    request(options, function(error, r) {
      if (error) {
        ...
        return reject(new Error("Ontsc Box at URL " + url + " doesn't exist. If you believe this is an error, please contact Ontsc support."));
...
function readBoxConfig(destination) {
  var possibleConfigs = [
    path.join(destination, "ontsc-box.json"),
    path.join(destination, "ontsc-init.json")
  ];
...
function cleanupUnpack(boxConfig, destination) {
  ...
  needingRemoval.push("ontsc-box.json");
  needingRemoval.push("ontsc-init.json");
