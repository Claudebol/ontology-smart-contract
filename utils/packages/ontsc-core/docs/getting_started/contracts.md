...
Now let's look at the Javascript object called `MetaCoin` provided for us by Ontsc and Ether Pudding, made available within our frontend:
...
**Warning:** This will only work successfully for contracts that have been deployed using `ontsc migrate` and are set to be deployed within your [project configuration](/advanced/configuration). This function will throw an error if your contract does not meet this criteria.
