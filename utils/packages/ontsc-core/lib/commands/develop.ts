...
var command = {
  ...
  run: function (options, done) {
    ...
    var warning =
      ...
      "This mnemonic was created for you by Ontsc. It is not secure.\n" +
    ...
    Develop.connectOrStart(ipcOptions, testrpcOptions, function(started) {
      ...
      if (started) {
        config.logger.log(`Ontsc Develop started at ${url}`);
        config.logger.log();
        ...
        config.logger.log(`Connected to existing Ontsc Develop session at ${url}`);
        config.logger.log();
      }
