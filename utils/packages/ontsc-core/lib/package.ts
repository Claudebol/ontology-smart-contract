...
var Profiler = require("ontsc-compile/profiler");
...
var Package = {
  ...
  publish: function(options, callback) {
    ...
    if (!ropsten) {
      return callback(new OntscError("You need to have a `ropsten` network configured in order to publish to the Ethereum Package Registry. See the following link for an example configuration:" + OS.EOL + OS.EOL + "    http://truffleframework.com/tutorials/using-infura-custom-provider" + OS.EOL));
    }
    ...
    Networks.asURIs(options, deployed_networks, function(err, result) {
      ...
      if(!files.length){
        var msg = "Could not locate any publishable artifacts in " +
                  ...
                  "Run `ontsc compile` before publishing.";
