...
try {
  ...
  throw new Error("Fatal: Error parsing arguments; please contact the Ontsc developers for help.");
}
...
TestRPCMixin.prototype.connect = function(supervisor, socket) {
  ...
  this.ready.then(function() {
    supervisor.emit(socket, 'ontsc.ready');
  });
}
...
// constructor - takes Logger instance and message key (e.g. `ontsc.ipc.log`)
...
supervisor.use(new LoggerMixin(ipcLogger, 'ontsc.ipc.log'));
supervisor.use(new LoggerMixin(testrpcLogger, 'ontsc.testrpc.log'));
supervisor.start();
