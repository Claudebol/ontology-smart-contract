...
First, set up a new web3 provider instance and initialize your contract, then `require("ontsc-contract")`. The input to the `contract` function is a JSON blob defined by [ontsc-contract-schema](https://github.com/claudebol/ontsc-contract-schema). This JSON blob is structured in a way that can be passed to all ontsc-related projects.

```javascript
var provider = new Web3.providers.HttpProvider("http://localhost:8545");
var contract = require("ontsc-contract");
...
In your `head` element, include Web3 and then include ontsc-contract:

```
<script type="text/javascript" src="./path/to/web3.min.js"></script>
<script type="text/javascript" src="./dist/ontsc-contract.min.js"></script>
```

Alternatively, you can use the non-minified versions for easier debugging.

With this usage, `ontsc-contract` will be available via the `OntscContract` object:

```
var MyContract = OntscContract(...);
```

### Full Example

Let's use `ontsc-contract` with an example contract from [Dapps For Beginners](https://dappsforbeginners.wordpress.com/tutorials/your-first-dapp/). In this case, the abstraction has been saved to a `.sol.js` file by [ontsc-artifactor](https://github.com/claudebol/ontsc-artifactor):

```javascript
// Require the package that was previosly saved by ontsc-artifactor
...
// how ontsc-contract allows for easy control flow.
...
MetaCoin.at(contract_address).then(function(instance) {
  ...
  // This code block will not be executed until ontsc-contract has verified
  // the transaction has been processed and it is included in a mined block.
  // ontsc-contract will error if the transaction hasn't been processed in 120 seconds.
...
Even more helpful, however is we *don't even need* to use `.call` when a function is marked as `constant`, because `ontsc-contract` will automatically know that that function can only be interacted with via a call:
...
This package is the result of breaking up EtherPudding into multiple modules. Tests currently reside within [ontsc-artifactor](https://github.com/claudebol/ontsc-artifactor) but will soon move here.
